...
for i, sim in enumerate(sim_vector):
    if sim >= threshold:
        a_start = (start_sample + i * segment_sec * sr) / sr
        a_end = a_start + segment_sec
        b_start = (i * segment_sec * sr) / sr
        b_end = b_start + segment_sec
        good_segments.append((a_start, a_end, b_start, b_end, sim))

print("===== [결과 요약] =====")
for idx, (a_s, a_e, b_s, b_e, sim) in enumerate(good_segments):
    print(f"구간 {idx+1}: A노래 {a_s:.1f}~{a_e:.1f}초 / B노래 {b_s:.1f}~{b_e:.1f}초 | 유사도 {sim*100:.1f}%")
